...
const applySearch = (value) => {
    router.get(route('admin.financial.expenses.index'), { search: value || undefined }, {
        preserveState: true,
        preserveScroll: true,
        replace: true,
    });
...
watch(search, (value) => {
    ...
    searchTimeout = setTimeout(() => applySearch(value), 300);
});

const deleting = ref(null);

const deleteExpense = () => {
    router.delete(route('admin.financial.expenses.destroy', deleting.value.id), {
        preserveScroll: true,
        onSuccess: () => {
            deleting.value = null;
        },
...
const toggleActive = (expense) => {
    router.patch(route('admin.financial.expenses.toggle-active', expense.id), {}, { preserveScroll: true });
};
</script>
    ...
    <AdminLayout>
        ...
        <div class="space-y-6">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h1 class="text-2xl font-bold tracking-tight text-brand-950">Despesas fixas</h1>
                ...
                <Link
                    ...
                    :href="route('admin.financial.expenses.create')"
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200">
                        <thead class="bg-slate-50">
                            <tr>
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Despesa</th>
                        ...
                        <tbody class="divide-y divide-slate-100 bg-white">
                            <tr v-for="expense in expenses.data" :key="expense.id" class="hover:bg-slate-50">
                                <td class="px-4 py-3">
                                    ...
                                    <p class="mt-0.5 text-xs text-slate-500">{{ expense.notes }}</p>
                                </td>
                                <td class="hidden px-4 py-3 md:table-cell">
                                    <span class="inline-flex items-center gap-2">
                                        <span class="h-2 w-2 rounded-full" :style="{ backgroundColor: expense.category?.color ?? '#94a3b8' }"></span>
                                        <span class="text-sm text-slate-600">{{ expense.category?.name ?? 'Sem categoria' }}</span>
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-sm text-slate-600">Dia {{ expense.due_day }}</td>
                                <td class="px-4 py-3 text-right text-sm font-bold text-slate-900">{{ formatCurrency(expense.amount) }}</td>
                                <td class="px-4 py-3">
                                    <Badge :color="expense.active ? 'green' : 'gray'">{{ expense.active ? 'Ativa' : 'Inativa' }}</Badge>
                                </td>
                                <td class="px-4 py-3">
                                    <div class="flex items-center justify-end gap-1">
                                        ...
                                        >
                                            <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                                                <path v-if="expense.active" stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                            ...
                            <tr v-if="expenses.data.length === 0">
                                <td colspan="6" class="px-4 py-3">
                                    <EmptyState
                                        ...
                                        description="Cadastre sua primeira despesa fixa."
                                    />
                                </td>
                            </tr>
                        </tbody>
        ...
        <ConfirmModal
            ...
            :message="`Esta ação removerá a despesa ${deleting?.description}. Continuar?`"
